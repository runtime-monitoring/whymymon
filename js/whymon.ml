...
module Whymon = struct
  ...
  let check_sig js_sig = false
    (* try *)
    (*   let str_sig = Js_of_ocaml.Js.to_string js_sig in *)
    (*   if String.is_empty str_sig then Js_of_ocaml__Js.string "empty" *)
    (*   else *)
    (*     (Base.Hashtbl.clear Pred.Sig.table; *)
    (*      Other_parser.Sig.parse_from_string str_sig; *)
    (*      Js_of_ocaml__Js.string "ok") *)
    (* with _ -> Js_of_ocaml__Js.string "error" *)

  (* let check_formula js_formula = *)
  (*   try *)
  (*     let str_f = Js_of_ocaml.Js.to_string js_formula in *)
  (*     if String.is_empty str_f then Js_of_ocaml__Js.string "empty" *)
  ...
  (*        | Finished -> (None, "ok") *)
  (*        | Skipped _ -> (None, "error") *)
  (*        | Processed pb -> *)
  (*           if (Option.is_none last_ts_opt) || pb.ts >= (Option.value_exn last_ts_opt) then *)
  (*             (Some(pb.ts), "ok") *)
  (*           else (None, "error")) *)
  (*     with _ -> (None, "error") in *)
  (*   let str_log = Js_of_ocaml.Js.to_string js_log in *)
  (*   if String.is_empty str_log then Js_of_ocaml__Js.string "empty" *)
  (*   else (let str_dbs = List.map (List.filter (String.split str_log ~on:'@') ~f:(fun s -> not (String.is_empty s))) *)
  (*                         ~f:(fun s -> "@" ^ s) in *)
  (*         List.fold_until str_dbs ~init:(None, "empty") *)
  (*           ~f:(fun (last_ts_opt, s) str_db -> *)
  (*             let (last_ts_opt, s) = check_step last_ts_opt str_db in *)
  (*             if String.equal s "ok" then Continue (last_ts_opt, s) *)
  (*             else Stop(Js_of_ocaml__Js.string s)) ~finish:(fun (_, s) -> Js_of_ocaml__Js.string s)) *)
